...
Cut_gpu* createCutsOfZeroHalf(Cut_gpu *h_cut, Cut_gpu_aux *cut_aux, int *h_solution, int nCuts, int precision, int nThreads,int nBlocks, int szPerSet)
{
    ...
    for(i=0; i<nThreads*nBlocks; i++)
    {
        if(h_solution[i*szPerSet]!=-1)
        {
            ...
            violation[c_aux] = 0;
            memset(Coef1,0,sizeof(int)*h_cut->numberVariables);
            c1 = h_solution[i*szPerSet];
            c2 = h_solution[i*szPerSet + 1];
            for(j = h_cut->ElementsConstraints[c1]; j< h_cut->ElementsConstraints[c1 + 1]; j++)
            {
                el = h_cut->Elements[j];
                Coef1[el] += h_cut->Coefficients[j];
            }
            for(j = h_cut->ElementsConstraints[c2]; j< h_cut->ElementsConstraints[c2 + 1]; j++)
            {
                el = h_cut->Elements[j];
                Coef1[el] += h_cut->Coefficients[j];
            }
            rhs = h_cut->rightSide[c1] + h_cut->rightSide[c2];
            for(j=0; j<h_cut->numberVariables; j++)
            {
                if(Coef1[j]<0)
                {
